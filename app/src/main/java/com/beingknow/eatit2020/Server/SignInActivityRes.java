...
public class SignInActivityRes extends AppCompatActivity implements View.OnClickListener {
  ...
  /*  public void buttonSignIn() {
        ...
        phoneNumber = activitySignInResBinding.etPhoneNumber.getText().toString();
        password = activitySignInResBinding.etPassword.getText().toString();
        ...
        table_users.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                //check if user not exit in database
                if (dataSnapshot.child(activitySignInResBinding.etPhoneNumber.getText().toString()).exists()) {
                    //Get User Information
                    mDialog.dismiss();
                    Users users = dataSnapshot.child(activitySignInResBinding.etPhoneNumber.getText().toString()).getValue(Users.class);
                    users.setPhone(activitySignInResBinding.etPhoneNumber.getText().toString());
                    if (Boolean.parseBoolean(users.getIsStaff())) {
                        if (users.getPassword().equals(activitySignInResBinding.etPassword.getText().toString())) {
                            Toast.makeText(SignInActivityRes.this, "Sign In Successfully !", Toast.LENGTH_SHORT).show();
                           *//* Intent intent = new Intent(SignInActivityRes.this, HomeActivity.class);
